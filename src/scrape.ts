  ...
  seasonLoop: while (season < 4) {
    ...
    pageLoop: while (hasNextPage) {
      ...
      const res = await AnimeService.animeSeasonYearSeasonGet({
        ...
        nsfw: true,
        fields:
          "id,title,main_picture,alternative_titles,start_date,end_date,synopsis,mean,rank,num_list_users,num_scoring_users,nsfw,created_at,updated_at,media_type,status,genres,my_list_status,num_episodes,start_season,broadcast,source,average_episode_duration,rating,pictures,background,related_anime,related_manga,recommendations,studios,statistics",
      }).catch((err) => {
